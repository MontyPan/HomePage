package us.dontcareabout.homePage.client.layer.ipLog;
...
import us.dontcareabout.homePage.client.vo.ipLog.NamePeriod;
...
public class IpTimeline extends VerticalLayoutContainer {
	...
	public IpTimeline(Map<String, IpData> ipMap) {
		add(new Timeline(ipMap), V_1x_1);
...
class Timeline extends LayerContainer {
	...
	Timeline(Map<String, IpData> ipMap) {
		int bgIndex = 0;
		...
		for (String ip : ipMap.keySet()) {
			...
			allName.addAll(ipData.getNameSet());
		}
	...
	private static int periodWidth(Date start, Date end) {
		return (int)(
			...
			/ (60 * 60 * 1000) + IpData.VALIDITY_HOUR
		) * HOUR_W;
	}
		...
		class NameLayer extends LayerSprite {
			NameLayer(List<NamePeriod> periodList) {
				periodList.forEach(p -> {
					...
					n.resize(periodWidth(p.start, p.getEnd().asDate()), H);
					add(n);
				});
			}
		}
